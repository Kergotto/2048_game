...
var global = arguments[3];
'use strict';
if (typeof structuredClone === 'undefined') global.structuredClone = (obj)=>JSON.parse(JSON.stringify(obj));
class Game {
    ...
    #move(getNextCell, reverse = false) {
        ...
        const loop = (callback)=>{
            for(let i = 0; i < 4; i++){
                ...
                for (const j of line)callback(i, j);
            }
        };
        loop((i, j)=>{
            const [row, col] = this.status === 'playing' ? [
                i,
                j
            ] : [];
    ...
    addNewCell() {
        ...
        for(let row = 0; row < 4; row++){
            for(let col = 0; col < 4; col++)if (this.cells[row][col] === 0) emptyCells.push([
                row,
                col
            ]);
        }
...
module.exports = Game; // Code to force win:
 //document.dispatchEvent(new CustomEvent('win'));

},{}]},["huHYX","3Aj1C"], "3Aj1C", "parcelRequireaa7d", {})

//# sourceMappingURL=2048_game.4841ef46.js.map
